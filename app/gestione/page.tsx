...
import { Plus, Trash2, LogOut, FileSpreadsheet, Wine, Share2, Copy, Check, Users, MessageCircle, Save, Archive, Settings, ExternalLink, Globe } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  async function checkUser() {
    ...
    if (!user) {
      ...
      // Crea il profilo se è un nuovo cliente
  ...
  async function fetchData(uid: string) {
    ...
    if (currentFairId) {
      const { data: leadsData } = await supabase
        .from('leads')
        .select('*, tastings (note, wines (wine_name))')
  ...
  const saveSettings = async () => {
    if (!userId) return;
    await supabase.from('wineries').upsert({ id: userId, name: wineryName, website_url: websiteUrl });
  ...
  const getWaLink = (lead: any) => {
    ...
    const msg = `Ciao ${lead.first_name}! Sono della cantina ${wineryName}. Grazie per averci visitato al ${fairName}. Ecco il nostro sito: ${websiteUrl}`;
    return `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`;
  };

  if (loading) return <div className="min-h-screen bg-white flex items-center justify-center font-bold text-slate-400 italic">Caricamento Pannello... 🍷</div>;
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans pb-32">
      ...
      <nav className="bg-white border-b p-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={() => supabase.auth.signOut().then(() => router.push('/login'))} className="bg-slate-50 p-2 rounded-xl text-slate-400 border border-slate-100"><LogOut size={20}/></button>
        </div>
      </nav>
      ...
      <main className="p-4 max-w-2xl mx-auto mt-4 space-y-6">
        ...
        {activeTab === 'leads' && (
          <div className="space-y-4 animate-in fade-in">
            <h2 className="text-xl font-black italic px-2">NUOVE DEGUSTAZIONI</h2>
            {leads.filter(l => !l.is_archived).length === 0 ? (
              <div className="text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 text-slate-400 italic">In attesa di visitatori...</div>
            ) : (
              leads.filter(l => !l.is_archived).map(lead => (
                <div key={lead.id} className={`bg-white p-6 rounded-[2.5rem] shadow-sm border-2 transition-all ${lead.is_contacted ? 'border-green-100' : 'border-slate-50 shadow-md'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-bold text-lg leading-none mb-1">{lead.first_name} {lead.last_name}</h3>
                    ...
                    <div className="flex gap-2">
                      ...
                      <a href={getWaLink(lead)} onClick={() => markAsContacted(lead.id)} target="_blank" className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-bold text-xs transition-all active:scale-95 ${lead.is_contacted ? 'bg-green-100 text-green-700' : 'bg-green-500 text-white shadow-lg'}`}>
                  ...
                  <div className="bg-slate-50 rounded-[1.5rem] p-4 border border-slate-100 space-y-3">
                    ...
                    {lead.general_notes && <p className="text-xs text-slate-600 bg-amber-50 p-3 rounded-xl border border-amber-100 italic"><strong>Memo Stand:</strong> {lead.general_notes}</p>}
                  </div>
                </div>
              ))
            )}
        ...
        {activeTab === 'config' && (
          <div className="space-y-6 animate-in slide-in-from-right">
            ...
            <div className="bg-white p-8 rounded-[3rem] border-2 border-slate-100 flex flex-col items-center gap-6 text-center shadow-sm">
              <div className="bg-white p-4 rounded-[2.5rem] shadow-xl border border-slate-50">
              ...
              <div className="w-full space-y-3">
                ...
                <a href={publicLink} target="_blank" className="w-full bg-slate-100 text-slate-600 font-bold py-3 rounded-2xl text-[10px] uppercase tracking-widest flex items-center justify-center gap-2">
                    <ExternalLink size={14}/> Apri Form Stand
                </a>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-200 space-y-4">
              ...
              <div className="space-y-4">
                ...
                <input placeholder="Sito Web (es: https://tuosito.it)" className="w-full p-4 bg-slate-50 rounded-2xl outline-none text-blue-600" value={websiteUrl} onChange={e => setWebsiteUrl(e.target.value)} />
